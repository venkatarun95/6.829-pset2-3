...
impl AggMeasurement {
    /// Will report a measurement at-most once per (reporting_interval * rtt) seconds
    ...
    pub fn report(
        ...
        if rtt != 0 {
            self.rtt = rtt;
            self.srtt = self.srtt_alpha * rtt as f32 + (1. - self.srtt_alpha) * self.srtt;
        }

        if now > 0 && self.last_report_time < now - (self.srtt * self.reporting_interval) as u64 {
            let res = (
                ReportStatus::Report,
                false,
                self.acked,
