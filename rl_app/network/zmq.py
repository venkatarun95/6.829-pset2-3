...
import zmq
from rl_app.network.serializer import get_serializer, get_deserializer
...
class ZmqSocket(object):
  ...
  def establish(self):
    ...
    if self.bind:
      # if self._verbose:
      #   print('[{}] binding to {}', self.socket_type, self.address)
      self._socket.bind(self.address)
    else:
      # if self._verbose:
      #   print('[{}] connecting to {}', self.socket_type, self.address)
...
class ZmqOneTimeReceiver(ZmqServer):
  """
        Uses REQ / REP to send and receive data,
        i.e. a client that only expects b'ack'
        by default binds
